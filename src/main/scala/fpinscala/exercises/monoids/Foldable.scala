...
import fpinscala.exercises.monoids.Monoid.endoMonoid
...
trait Foldable[F[_]]:
  ...
  extension [A](as: F[A])
    ...
    def combineAll(using ma: Monoid[A]): A = as.foldLeft(ma.empty)(ma.combine)
...
object Foldable:
  ...
  given Foldable[LazyList] with
    extension [A](as: LazyList[A])
      ...
      override def foldLeft[B](acc: B)(f: (B, A) => B) =
        as.foldLeft(acc)(f)
      override def foldMap[B](f: A => B)(using mb: Monoid[B]): B =
        as.foldLeft(mb.empty)((a, b) => mb.combine(a, f(b)))
  ...
  given Foldable[Option] with
    extension [A](as: Option[A])
      override def foldRight[B](acc: B)(f: (A, B) => B) =
        as.fold(acc)(a => f(a, acc))
      override def foldLeft[B](acc: B)(f: (B, A) => B) =
        as.fold(acc)(a => f(acc, a))
      override def foldMap[B](f: A => B)(using mb: Monoid[B]): B =
        as.fold(mb.empty)(a => mb.combine(f(a), mb.empty))
